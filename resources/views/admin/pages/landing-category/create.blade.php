...
@section('content')
   <!-- Content Header (Page header) -->
    ...
    <section class="content">
        <form action="{{ route('admin.landing.category.store') }}" role="form" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-12">
                    <div class="btn-group">
                        <button type="submit" class="btn btn-primary btn-sm mr-1" title="{{__('Save')}}"><i class="fa fa-save"></i> {{__('Save')}}</button>
                        <a href="{{ route('admin.landing.category.index') }}" class="btn btn-default btn-sm" title="{{__('Back')}}"><i class="fa fa-undo"></i> {{__('Back')}}</a>
                    </div>
                </div>
                <div class="col-md-12">
                    @if ($errors->any())
