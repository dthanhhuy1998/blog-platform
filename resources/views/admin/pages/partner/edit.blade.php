...
@section('content')
   <!-- Content Header (Page header) -->
    ...
    <section class="content">
        <form action="{{ route('admin.theme.partner.postEdit') }}" role="form" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="btn-group">
                        ...
                        <a href="{{ route('admin.theme.partner.index') }}" class="btn btn-default btn-sm" title="Quay lại"><i class="fa fa-undo"></i> Quay lại</a>
                    </div>
                </div>
                <div class="col-md-12">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <div class="preview-image">
                                    <img src="@if(!empty($partner->image)) {{ asset('storage/' . $partner->image) }} @else {{ asset('storage/uploads/default.png') }} @endif" alt="Image" id="preview">
                                </div>
                                <input type="file" onchange="filePreview(event)" class="form-control" name="image">
                            </div>
                            <div class="form-group">
                                <label>Liên kết</label>
                                <input type="text" class="form-control" value="{{ $partner->link }}" name="link">
                            </div>
                            <div class="form-group">
                                <label>Vị trí</label>
                                <input type="number" class="form-control" value="{{ $partner->sort_order }}" name="sortOrder">
                            </div>
                            <div class="form-group">
                                <label>Hiển trị trang chủ</label>
                                <div class="radio">
                                    <label>
                                        <input type="radio" name="status" value="1" @if($partner->status == 1) checked @endif>
                                        Có
                                    </label>
                                </div>
                                <div class="radio">
                                    <label>
                                        <input type="radio" name="status" value="0" @if($partner->status == 0) checked @endif>
                                        Không
                                    </label>
                                </div>
                            </div>
