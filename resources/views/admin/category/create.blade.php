...
@section('content')
   <!-- Content Header (Page header) -->
    ...
    <section class="content">
        <form action="{{ route('admin.categories.store') }}" role="form" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-12">
                    <div class="btn-group">
                        <button type="submit" class="btn btn-primary btn-sm mr-1" title="{{__('Save')}}"><i class="fa fa-save"></i> {{__('Save')}}</button>
                        <a href="{{ route('admin.categories.index') }}" class="btn btn-default btn-sm" title="{{__('Cancel')}}"><i class="fa fa-undo"></i> {{__('Cancel')}}</a>
                    </div>
                </div>
                <div class="col-md-12">
                    @if ($errors->any())
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="general">
                                <div class="form-group @error('categoryName') has-error @enderror">
                                    ...
                                    <input type="text" class="form-control" placeholder="{{__('Enter category name')}}" value="{{ old('categoryName') }}" name="categoryName">
                                    @error('categoryName')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" rows="8" class="form-control textarea" placeholder="{{__('Enter description')}}">{{ old('description') }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>{{__('Image')}}</label>
                                    <div class="preview-image">
                                        <img src="{{ asset('images/default.jpg') }}" alt="Image" id="preview">
                                    </div>
                                    <input type="file" onchange="filePreview(event)" class="form-control" name="file">
                                </div>
                                <div class="form-group">
                                    <label>{{__('Position')}}</label>
                                    <input type="number" class="form-control" placeholder="{{__('Position')}}" value="0" name="sortOrder">
                                </div>
                                <div class="form-group">
                            ...
                            <div class="tab-pane" id="seo">
                                <div class="form-group @error('metaTitle') has-error @enderror">
                                    ...
                                    <input type="text" class="form-control" placeholder="{{__('Enter meta title')}}" value="{{ old('metaTitle') }}" name="metaTitle">
                                    @error('metaTitle')
                                        <span class="help-block">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="5" cols="40" name="metaDescription" placeholder="{{__('Enter meta description')}}">{{ old('metaDescription') }}</textarea>
                                </div>
                                <div class="form-groupr">
                                    <label>{{__('Meta Keywords')}}</label>
                                    <textarea class="form-control" rows="5" cols="30" name="metaTagKeywords" placeholder="{{__('Enter meta keywords')}}">{{ old('metaTagKeywords') }}</textarea>
                                </div>
                            </div>
                            <!-- /.tab-pane -->
                        </div>
